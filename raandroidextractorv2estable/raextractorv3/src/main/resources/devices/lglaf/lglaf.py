...
try: import usb.core, usb.util
    ...
    print('usbcore error import laf')
...
try:
    #from ControladorLAF.Lglaf
    import laf_crypto
    #from ControladorLAF.Lglaf.laf_crypto import encrypt_kilo_challenge
...
parser.add_argument("--usb", action='store_true', help="Enable usb LG K4 and others")
...
def main():
    ...
    if args.usb:
        pwd = os.getcwd()
        cmd = os.path.realpath('.')+"/usbswitcher.c"
        print ("Activando usb LG\n")
        subprocess.call(["gcc",cmd,"-lusb"]) #compilacion
        subprocess.call("./a.out") #ejecucion
