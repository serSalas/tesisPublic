 ...
 *
 */
public class DesbloquearPorDeletePass extends DesbloquearPantalla
{
	private static int estadoDesbloquearPantalla;
	private IDevice disp;


	public DesbloquearPorDeletePass(Dispositivo dispositivo) {
		super(dispositivo);
						...
						{	
							for(IDevice device : adb.getDevices())
							{
								...
								&& dispositivo.getNumeroCompilacion().equals(Propiedades.getPropiedades("ro.build.display.id")))
								{
									...
									Thread.sleep(5000);
									Process p1 = Runtime.getRuntime().exec("adb shell rm /data/system/password.key");
									Thread.sleep(1000);
